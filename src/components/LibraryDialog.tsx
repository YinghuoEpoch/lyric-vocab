...
import { asBookFile, fetchBook, NoEpub, type DownloadProgress } from '../library/download'
...
function describeError(e: unknown): string {
  ...
  if (e instanceof NoEpub) {
    return '这本书取不到，可能书库那边没存这个格式。换一本试试'
  }
...
export function LibraryDialog({ open, onClose, onImport }: LibraryDialogProps) {
  ...
  /**
   * 正在下载的是哪一本，同时也用来禁掉其它条目。
   * 用「源:定位」而不是编号 —— 两个源的编号各排各的，会撞。
   */
  ...
  const bookKey = (b: CatalogBook) => `${b.source}:${b.ref}`
  ...
  const handlePick = (book: CatalogBook) => {
    ...
    void (async () => {
      try {
        const bytes = await fetchBook(book, setProgress)
        onImport(asBookFile(bytes, book))
        onClose()
      } catch (e) {
            ...
            <div className="-mx-1 min-h-0 flex-1 overflow-y-auto px-1">
              {query.trim() === '' ? (
                <p className="py-6 text-center text-xs leading-relaxed text-ink-muted">
                  共 {catalogSize().toLocaleString()} 本版权已过期的免费英文书
                  <br />
                  来自古登堡计划的美国站与澳洲站
                  <br />
